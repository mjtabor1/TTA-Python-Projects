...
def center_window(self, w, h):  # pass in the tkinter frame (master) reference and the w and h
    ...
    screen_width = self.master.winfo_screenwidth()  # method from tkinter module to store user's screen width.
    screen_height = self.master.winfo_screenheight()  # method from tkinter to store user's screen height
    # calculate x and y coordinates to paint the app centered on the user's screen
    x = int((screen_width / 2) - (w / 2))
    y = int((screen_height / 2) - (h / 2))
    # equation that is returned to main window(self.master) to give it some dimensions so it can center the GUI.
...
def ask_quit(self):
    # tkinter class(messagebox) and method(askokcancel) which will help us close the GUI
    if messagebox.askokcancel("Exit program", "Okay to exit application?"):
        # This closes app
        self.master.destroy()
        os._exit(0)  # this takes all of our widgets and deletes it from memory. Good practice.
...
def first_run(self):
    ...
    with conn:
        cur = conn.cursor()
        cur, count = count_records(cur)  # passing cur into our count_records function. Cur & Count will be returned
        if count < 1:
            cur.execute(
                """INSERT INTO tbl_phonebook (col_fname,col_lname,
...
def count_records(cur):
    count = ""
    cur.execute("""SELECT COUNT(*) FROM tbl_phonebook""")  # accessing the count of entries from the dB
    count = cur.fetchone()[0]  # extract the data from the [0] index and pass it into count
    return cur, count
...
def onSelect(self, event):
    ...
    varList = event.widget  # accesses the tkintr event
    select = varList.curselection()[0]  # gets the index of what was selected
    value = varList.get(select)  # list box gets the text from whatever index was selected
    conn = sqlite3.connect('db_phonebook.db')
    with conn:
        cursor = conn.cursor()
    ...
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and (
        ...
        with conn:
            ...
            if chkName == 0:  # if this is 0 then there is no existance of the fullname and we can add new data
                ...
                cursor.execute(
                    """INSERT INTO tbl_phonebook (col_fname,col_lname,col_fullname,col_phone,col_email) 
                    VALUES (?,?,?,?,?)""",
    ...
    with conn:
        ...
        if count > 1:
            ...
            confirm = messagebox.showerror("Last Record Error",
                                           "({}) is the last record in the database and cannot be "
                                           "deleted at this time. \n\nPlease add another record first "
                                           "before you can delete ({}).".format(
            ...
            if count == 0 or count2 == 0:  # if proposed changes are not already in the database, then proceed
                ...
                if response:
                    ...
                    with conn:
                        cursor = conn.cursor()
                        cursor.execute(
                            """UPDATE tbl_phonebook SET col_phone = '{0}',col_email = '{1}' 
                            WHERE col_fullname = '{2}'""".format(
                                var_phone, var_email, var_value))
                        onClear(self)
                        conn.commit()
                else:
